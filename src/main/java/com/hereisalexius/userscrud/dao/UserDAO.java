...
public class UserDAO {
	...
	public List<User> list() {
		...
		return (List<User>) jdbcTemplate.query(query, new UserRowMapper());
	}
	...
	public List<User> findAllInFullname(String data) {
		...
		String query = "SELECT * FROM USERS " + "WHERE LOWER(FULLNAME) " + "LIKE LOWER('%?%')";

		return (List<User>) jdbcTemplate.query(query, new Object[] { data }, new UserRowMapper());
	}

	@Deprecated
	public List<User> streamFindAllInFullname(String data) {
		return list().stream().filter(user -> user.getFullName().toLowerCase().contains(data.toLowerCase()))
